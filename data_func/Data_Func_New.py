...
##smile,加上指标类型和多loss,蛋白质tape编码,无指标类型,大于等于0.5为正样本,mol指纹+SMILE
##11基础上加pssm，model51

class Data_Func_New(Dataset):

    def __init__(self, data, mode, ind,whole=False):
        super(Data_Func_New,self).__init__()
        self.data_set = data
        self.mode = mode
        self.dict = {'Ki': 1, 'Potency': 2, 'IC50': 3, 'EC50': 4, 'Kd': 5, 'AC50': 6}
        self.whole = whole
        if mode=='inf':
            data_dir = Path(ind).resolve()
            if whole:
                self.pro = np.load(data_dir / 'inf_pro_feature_whole.npz',
                                   allow_pickle=True)
                self.pssm = np.load(data_dir / 'pssm_400_whole.npz',
                                    allow_pickle=True)
            else:
                self.pro = np.load(data_dir / 'inf_pro_feature.npz', allow_pickle=True)
                self.pssm = np.load(data_dir / 'pssm_400.npz', allow_pickle=True)

            self.mol = [token.strip().split('_') for token in
                        open(data_dir / ('mol_' + mode))]
            self.data = self.process_inf()
        else:
            if ind == '28':
                self.pro = np.load('data_test/28/tf_feature.npz', allow_pickle=True)
                self.pssm = 1
            else:
                self.pro = np.load('data_test/all_pro_feature.npz', allow_pickle=True)
                self.pssm = np.load('pssm_400.npz',allow_pickle=True)
                if ind =='18_2':
                    self.dt_pro = np.load('data_test/'+ind+'/P0DTD1_pro.npz', allow_pickle=True)
                    self.dt_pssm = np.load('data_test/'+ind+'/P0DTD1_pssm.npz', allow_pickle=True)
            self.mol = [token.strip().split('_') for token in open('data_test/'+ind+'/mol_'+mode)]
    ...
    def process_inf(self):
        ...
        for i, line in enumerate(self.data_set):
            ...
            # >>> 统一用 9AYL，不再硬编码 P0DTD1 <<<
            pro_key = '9AYL'                    # 真实存在的键
            if self.whole:
                pro = np.reshape(self.pro[pro_key], [1])[0]['avg']
                pssm = self.pssm[pro_key + '_whole']
            else:
                pro = np.reshape(self.pro[pro_key], [1])[0]['avg']
                pssm = self.pssm[pro_key]
...
class Data_Func_New_Metric(object):
    """
    用于单机或分布式训练的评价标准的类
    ...
    def __init__(self, trg_pad_idx=0):
        super(Data_Func_New_Metric, self).__init__()
