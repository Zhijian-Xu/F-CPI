...
class Pre_data(Dataset):
    ...
    def __init__(self, data_root, data_pre_func, batch_size,rank=-1, inf_data = None,whole=False):
        ...
        if data_root == 'inf':
            ...
            self.modes = ['train', 'valid', 'test'] if int(data_root[9:]) > 14 else ['train', 'valid']
    ...
    def get_data_loader(self, mode):
        ...
        ##collate_fn = eval('data_func.' + self.data_pre_func + '.generate_batch')
    ...
    def prepare(self, mode):
        #vocab = [token.strip().split('_') for token in open(os.path.join(self.root, mode + '_set'))]
        with open(self.root+'/'+mode + '_set') as f:

            vocab = [token.strip().split('_') for token in f]
        # vocab = []
        # for i,line in enumerate(open(self.root+'/'+mode + '_set')):
        #     if i >1:
        #         break
        #     vocab.append(line.strip().split('_'))
